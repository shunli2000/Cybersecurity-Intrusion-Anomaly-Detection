...
class BETHDataset(TensorDataset):
    ...
    def get_input_shape(
        self,
    ):  # note that does not return actual shape, but is used to configure model for categorical data
        num_classes = self.data.max(dim=0)[0] + 1
        num_classes[4] = (
            1011  # Manually set eventId range as 0-1011 (1010 is max value)
        )
        return num_classes
    ...
    def plot(self, dataset_list, label_list, prefix="dataset", suffix=""):
        ...
        sns.scatterplot(
            x=x1[:, 0], y=x2[:, 0], label=y, legend="full", s=5, palette=palette
        )
...
class GaussianDataset(TensorDataset):
    ...
    def __init__(self, split="train", subsample=0):
        if split == "train":
            self.data = torch.cat(
                [
                    0.2 * torch.randn(1000, 2) + torch.tensor([[1.0, 1.0]]),
                    0.2 * torch.randn(1000, 2) + torch.tensor([[-1.0, -1.0]]),
                ]
            )
            ...
            self.data = torch.cat(
                [
                    0.2 * torch.randn(100, 2) + torch.tensor([[1.0, 1.0]]),
                    0.2 * torch.randn(100, 2) + torch.tensor([[-1.0, -1.0]]),
                ]
            )
            self.labels = torch.tensor([0] * len(self.data))
        elif split == "test":
            anomalies = 0.2 * torch.randn(100, 2) + torch.tensor([[1.0, -1.0]])
            self.data = torch.cat(
                [0.2 * torch.randn(100, 2) + torch.tensor([[1.0, 1.0]]), anomalies]
            )
            self.labels = torch.cat(
                (
                    torch.tensor([0] * (len(self.data) - len(anomalies))),
                    torch.tensor([1] * len(anomalies)),
                )
            )
        else:
            raise Exception("Error: Invalid 'split' given")
    ...
    def plot(self, dataset_list, label_list, prefix="dataset", suffix=""):
        ...
        for i in range(len(dataset_list)):
            if type(dataset_list[i]) == GaussianDataset:
                plt.scatter(
                    x=dataset_list[i].tensors[0][:, 0],
                    y=dataset_list[i].tensors[0][:, 1],
                    color=colors[i],
                    label=label_list[i],
                    edgecolors=None,
                    linewidths=1,
